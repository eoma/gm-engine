...
MainComponentSerializer::MainComponentSerializer(Main *currentApp) : app(currentApp) {
	slots.connect(
		...
		add_component_creator(Framework::Camera::get_static_type(), [this](const Framework::EntityPtr &owner) {
			owner->create_component<Framework::Camera>(app->get_render_system(), app->get_texture_manager());
		});
		
		add_component_creator(Framework::Renderable::get_static_type(), [this](const Framework::EntityPtr &owner) {
			owner->create_component<Framework::Renderable>(app->get_render_system(), app->get_material_manager(), app->get_mesh_manager());
		});
		
		add_component_creator(Framework::Transform::get_static_type(), [this](const Framework::EntityPtr &owner) {
			owner->create_component<Framework::Transform>(app->get_scene_system());
		});
		
		add_component_creator(Framework::Light::get_static_type(), [this](const Framework::EntityPtr &owner) {
			owner->create_component<Framework::Light>(app->get_render_system());
		});
		
		add_component_creator(Framework::Tessellate::get_static_type(), [this](const Framework::EntityPtr &owner) {
			owner->create_component<Framework::Tessellate>(app->get_render_system());
		});
		
		add_component_creator(Framework::StandardPass::get_static_type(), [this](const Framework::EntityPtr &owner) {
