...
ComponentContainer<UserData>::~ComponentContainer()
{
	for(unsigned int i = 0; i < components.size(); i++)
		sign_ComponentRemoved(components[i]);
}
...
template<class UserData>
clan::Signal<void(std::shared_ptr<IComponent<UserData>>)> &ComponentContainer<UserData>::component_added() 
{ 
	return sign_ComponentAdded; 
}

template<class UserData>
clan::Signal<void(std::shared_ptr<IComponent<UserData>>)> &ComponentContainer<UserData>::component_removed() 
{ 
